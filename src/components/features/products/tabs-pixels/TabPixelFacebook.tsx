...
import { FacebookPixelForm } from "./forms";
import type { FacebookPixel } from "@/types/pixel";
import Modal from "@/components/widgets/modal";
import { cn } from "@/lib/utils";

interface TabPixelFacebookProps {
  productId: string;
}

export function TabPixelFacebook({ productId }: TabPixelFacebookProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isOpenEdit, setIsOpenEdit] = useState(false);
  const [selectedPixel, setSelectedPixel] = useState<FacebookPixel | null>(
    null
  );
  ...
  const pixels = pixelsData?.pixels.FACEBOOK_ADS || [];
  ...
  const renderCard = (pixels: FacebookPixel[]) => {
    return (
  ...
  return (
    <Fragment>
      <div className="flex flex-row items-center justify-between gap-4">
        <h3 className="text-lg font-semibold">Pixels do Facebook</h3>
      ...
      <div className="mt-6">
        {isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            Nenhum pixel do Facebook encontrado
          </div>
        ) : (
          renderCard(pixels)
        )}
      ...
      <Modal
        ...
        title="Adicionar Pixel do Facebook"
        description="Adicione seu pixel do Facebook"
      >
        <FacebookPixelForm productId={productId} onSuccess={handleSuccess} />
      </Modal>
      ...
      <Modal
        ...
        description="Edite seu pixel do Facebook"
      >
        {selectedPixel && (
          <FacebookPixelForm
            productId={productId}
