...
import { TikTokPixelForm } from "./forms";
import type { TikTokPixel } from "@/types/pixel";
import Modal from "@/components/widgets/modal";
import { cn } from "@/lib/utils";

interface TabPixelTiktokProps {
  productId: string;
}

export function TabPixelTiktok({ productId }: TabPixelTiktokProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isOpenEdit, setIsOpenEdit] = useState(false);
  const [selectedPixel, setSelectedPixel] = useState<TikTokPixel | null>(null);

  const { data: pixelsData, isLoading } = usePixels(productId);
  const deletePixelMutation = useDeletePixel(productId);

  const pixels = pixelsData?.pixels.TIKTOK_ADS || [];

  const handleEdit = (pixel: TikTokPixel) => {
  ...
  const renderCard = (pixels: TikTokPixel[]) => {
    return (
  ...
  return (
    <Fragment>
      <div className="flex flex-row items-center justify-between gap-4">
        <h3 className="text-lg font-semibold">Pixels do TikTok</h3>
      ...
      <div className="mt-6">
        {isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            Nenhum pixel do TikTok encontrado
          </div>
        ) : (
          renderCard(pixels)
        )}
      ...
      <Modal
        ...
        title="Adicionar Pixel do TikTok"
        description="Adicione seu pixel do TikTok"
      >
        <TikTokPixelForm productId={productId} onSuccess={handleSuccess} />
      </Modal>

      <Modal
        isOpen={isOpenEdit}
        onClose={handleCloseModals}
        title="Editar Pixel do TikTok"
        description="Edite seu pixel do TikTok"
      >
        {selectedPixel && (
          <TikTokPixelForm
            productId={productId}
